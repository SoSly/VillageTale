...
public class BringAnimalToButchery extends Behavior<Villager> {
    ...
    private BlockPos butcheryCenter;
    private int phaseTicks;
    private boolean hasLeadAttached;
    private Goal followGoal;

    public BringAnimalToButchery() {
        super(ImmutableMap.of(
                MemoryModuleTypes.WORK_ZONE.get(), MemoryStatus.VALUE_PRESENT,
                MemoryModuleTypes.FOUND_ENTITY.get(), MemoryStatus.VALUE_PRESENT,
    ...
    protected boolean checkExtraStartConditions(@NotNull ServerLevel level, @NotNull Villager villager) {
        ...
        Optional<Entity> foundEntityMemory = villager.getBrain().getMemory(MemoryModuleTypes.FOUND_ENTITY.get());
        if (foundEntityMemory.isEmpty() || !(foundEntityMemory.get() instanceof Animal)) {
            return false;
        }

        Animal animal = (Animal) foundEntityMemory.get();
    ...
    protected void start(@NotNull ServerLevel level, Villager villager, long gameTime) {
        ...
        butcheryCenter = butchery.getStartPosition();
        ...
        VillagerEquipmentSync.sendToNearbyPlayers(villager, InteractionHand.MAIN_HAND, lead.copy());

        villager.getBrain().setMemory(MemoryModuleType.WALK_TARGET,
                new WalkTarget(targetAnimal.blockPosition(), 0.5f, 0));
    }
    ...
    protected boolean canStillUse(@NotNull ServerLevel level, @NotNull Villager villager, long gameTime) {
        ...
        if (currentPhase == Phase.RELEASING && butchery.containsPosition(targetAnimal.blockPosition())) {
            return true;
        }
    ...
    private void tickLeashing(ServerLevel level, Villager villager) {
        ...
        if (phaseTicks == 10) {
            ...
            villager.getBrain().eraseMemory(MemoryModuleTypes.FOUND_ENTITY.get());
    ...
    private void tickLeading(ServerLevel level, Villager villager) {
        ...
        if (butchery.containsPosition(targetAnimal.blockPosition())) {
        ...
        if (!villager.getBrain().hasMemoryValue(MemoryModuleType.WALK_TARGET) || phaseTicks % 40 == 0) {
            BlockPos targetPos = findBestPathToButchery(villager.blockPosition(), butcheryCenter);
            villager.getBrain().setMemory(MemoryModuleType.WALK_TARGET,
    ...
    private BlockPos findBestPathToButchery(BlockPos from, BlockPos butcheryCenter) {
        double distance = from.distSqr(butcheryCenter);
        if (distance < 16) {
            return butcheryCenter;
        }

        double ratio = Math.sqrt(16 / distance);
        int targetX = from.getX() + (int) ((butcheryCenter.getX() - from.getX()) * ratio);
        int targetZ = from.getZ() + (int) ((butcheryCenter.getZ() - from.getZ()) * ratio);
